...
void init_particles(Particle* p, int N, unsigned int seed) {
    ...
    for (int i=0; i<N; ++i) {
        ...
        float fr3 = (float)rand()/RAND_MAX;
        float fr4 = (float)rand()/RAND_MAX;
        float fv1 = (float)rand()/RAND_MAX;
        float fv2 = (float)rand()/RAND_MAX;
        float fv3 = (float)rand()/RAND_MAX;
        float fv4 = (float)rand()/RAND_MAX;

        Particle q;
        q.x = fr1*2.0f - 1.0f; q.y = fr2*2.0f - 1.0f;
        q.z = fr3*2.0f - 1.0f; q.w = fr4*2.0f - 1.0f;

        q.vx = fv1*1.4f - 0.7f; q.vy = fv2*1.4f - 0.7f;
        q.vz = fv3*1.4f - 0.7f; q.vw = fv4*1.4f - 0.7f;

        q.hue = (float)rand()/RAND_MAX;
        q.radius = 1.5f + 1.5f*((float)rand()/RAND_MAX);
                      ...
                      int W, int H, float t, float focal4, float focal3)
{
    ...
    const float bounds = 1.15f, damp = 0.85f, accel = 0.20f;
