...
static void set_window_title(SDL_Window* win, float fps, int mode, TessSubmode sub, const Camera3* cam) {
    ...
    if (mode==MODE_TESSERACT) {
        const char* s = (sub==TESS_ROTATE? "tess_rotate" :
                      ...
                      int* mode, TessSubmode* sub)
{
    ...
    *W = 1280; *H = 720; *threads = -1; *seed = 12345u; *fpscap = 0;
...
int main(int argc, char** argv)
{
    ...
    SDL_Window* window = SDL_CreateWindow("Screensaver 4D (C)",
    ...
    while (running) {
        ...
        while (SDL_PollEvent(&e)) {
            ...
            if (e.type==SDL_KEYDOWN){
                ...
                if (e.key.keysym.sym==SDLK_m){ mouse_captured = !mouse_captured; SDL_SetRelativeMouseMode(mouse_captured?SDL_TRUE:SDL_FALSE); }
                if (e.key.keysym.sym==SDLK_r){ cam.px=cam.py=0; cam.pz=-5.5f; cam.yaw=cam.pitch=0; cam.fov_deg=70; }
            }
            ...
            if (e.type==SDL_MOUSEWHEEL){
                ...
                if (cam.fov_deg<30) cam.fov_deg=30;
                if (cam.fov_deg>110) cam.fov_deg=110;
            }
        }

        // dt
        Uint64 tnow = SDL_GetPerformanceCounter();
        double dt = (double)(tnow - tprev) / (double)freq;
        tprev = tnow; if (dt>0.1) dt=0.1;
        ...
        if (mode==MODE_PARTICLES){
            ...
            for (int i=0;i<N;++i){
                ...
                SDL_FRect r = { drawbuf[i].x-drawbuf[i].r*0.5f, drawbuf[i].y-drawbuf[i].r*0.5f, drawbuf[i].r, drawbuf[i].r };
        ...
        if (acc>=0.5){ float fps=(float)(frames/acc); fps_smoothed=0.7f*fps_smoothed+0.3f*fps;
            set_window_title(window, fps_smoothed, mode, sub, &cam); acc=0; frames=0; }

        if (fpscap>0){
            static Uint32 last=0; Uint32 now=SDL_GetTicks();
            Uint32 target=(Uint32)((1000.0f/(float)fpscap)+0.5f);
            if (now-last<target) SDL_Delay(target-(now-last)); last=SDL_GetTicks();
        }
    }
