                          ...
                          float focal4, const Camera3* cam)
{
    const int COUNT = 40;
    static int inited = 0;
    static TessInstance inst[COUNT];

    if (!inited){
        uint32_t seed = 123456u;
        for (int i=0;i<COUNT;++i){
            float rx = ((int)(lcg(&seed)%2000)-1000)/160.0f; // -6..+6 aprox
            ...
            inst[i] = (TessInstance){rx,ry,rz,rw, sc};
        }
        inited = 1;
    }
    ...
    for (int n=0;n<COUNT;++n){
        // no rotación 4D para “fijo”; pero sí leve respiración de w para sensación 4D
        float axw=0, ayz=0, axy=0, azw=0;
        float X[16],Y[16],Z[16],B[16];

        for (int i=0;i<16;++i){
            V4 v = base[i];
            v.x = v.x*inst[n].scale + inst[n].cx;
            v.y = v.y*inst[n].scale + inst[n].cy;
            v.z = v.z*inst[n].scale + inst[n].cz;
            v.w = v.w*inst[n].scale + inst[n].cw + 0.2f*sinf(t*0.6f + 0.7f*inst[n].cx); // “parpadeo” 4D
